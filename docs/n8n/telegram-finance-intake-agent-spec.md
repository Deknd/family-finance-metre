...
- на стороне intake API это поле трактуется как optional строковый correlation id, а не как отдельное UUID-ограничение DTO;
- если `request_id` передан, но не существует, сервер возвращает `422 VALIDATION_ERROR`;
- если `request_id` передан, но запись уже не находится в статусе `accepted`,
  сервер тоже возвращает `422 VALIDATION_ERROR`.
...
- `request_id` в примерах и правилах описан как server-generated UUID-строка,
  которая в callback возвращается без изменений и на intake-границе трактуется как correlation id;
