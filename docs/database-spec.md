...
После приема payload сервер должен выделить актуальные значения по конкретному человеку за конкретный месяц.

Для этого используется:

- `member_finance_snapshots`

Это уже не сырое сообщение, а нормализованная запись, с которой удобно считать агрегаты.
...
Хранит актуальное финансовое состояние конкретного члена семьи за конкретный месяц.
...
- одна запись = актуальный снимок по одному человеку за один месяц

Если приходит новый payload за тот же месяц, запись обновляется на основе более свежего `finance_submissions`.
...
3. Сервер обновляет или создает запись в `member_finance_snapshots`.
